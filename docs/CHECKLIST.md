...
- [x] Supplier order dashboard live at mwacoustic.com/orders (desktop-92 session, portal-orders 769eec9). Reviewed here: no secrets in the bundle, the Supabase key is the publishable browser key with authorization server-side behind a signed-in edge function, sign-in is email + password (15 char minimum) or an 8-digit emailed code, strict CSP, supabase-js pinned with Subresource Integrity, noindex,nofollow, absent from NAV, footer and sitemap, no order data baked into the page, and the local mock mode cannot be switched on from a real visit (it requires a localhost hostname over plain http as well as ?mock=1).
...
- Order dashboard UI copy is NEW and unapproved — see the 2026-09-16 entry in COPY-SOURCES.md. Read the screens once and approve or send edits.
- Order dashboard, known limitation: orders.js and orders.css are served without a cache-busting version, so after an update Beverly's browser may keep the old files for around 10 minutes. Acceptable for now; the fix is the same md5 suffix the stylesheet already uses (CSS_VER in build.py).
